...
class Solution:
    def subsetsWithDup(self, nums: List[int]) -> List[List[int]]:
        ...
        start = 0
        nums.sort()
        self.backtrack(nums,result,temp,count,start)
        return result

    def backtrack(self,nums,result,temp,count,start):
        ...
        for i in range(start,len(nums)):
            ...
            if i > start and nums[i] == nums[i-1]:
                continue
            temp.append(nums[i])
            self.backtrack(nums,result,temp,count,i+1)
            temp.pop()
```
...
class Solution:
    ...
    def nextPermutation(self, nums: List[int]) -> None:
        target = self.multipyplus(nums)
        # print(target)
        # target = 100 * nums[0] + 10 * nums[1] + nums[2]
        result = []
        temp = []
        start = 0
        length = len(nums)
        count = Counter(nums)
        self.backtrack(list(set(nums)),result,temp,count,target,length)
        # result = [int("".join(sublist)) for sublist in result]
        # print(result)
        result.sort(key = lambda x:"".join(str(x)))
        nums[:] = result[0] if result else sorted(nums)

    def backtrack(self,nums,result,temp,count,target,length):
        # if result : return 
        if len(temp) == length:
            # print(len(temp))
            a = self.multipyplus(temp)
            # a = 100 * temp[0] + 10 * temp[1] + temp[2]
            if a > target:
                # print(a)
                result.append(temp[:])
            return 
            ...
            for num in nums:
                if count[num] != 0:
                    temp.append(num)
                    count[num] -= 1
                    self.backtrack(nums,result,temp,count,target,length)
                    temp.pop()
                    count[num] += 1

    def multipyplus(self,nums):
        target = 0
        length = len(nums)
        for i in range(length):
            target += int("1"+ (length-1-i) * "0") * nums[i]
        return target
...
class Solution:
    def multiply(self, num1: str, num2: str) -> str:
        ...
        for i in range(len(num1)-1,-1,-1):
            mul = 1 * 10 ** (len(num1)-1-i)
            for j in range(len(num2)-1,-1,-1):
                result += int(num1[i]) *  mul * int(num2[j])
                mul = mul * 10
        return str(result)
```
    ...
    def deleteDuplicates(self, head: ListNode) -> ListNode:
        ...
        while pnode:
            if pnode.next and pnode.val == pnode.next.val:
                temp = pnode.val
                while pnode.next and pnode.next.val == temp:
                    pnode.next = pnode.next.next
                pre.next = pnode.next
                pnode = pre.next
            else:
...
class Solution:
    def fourSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        start = 0
        nums.sort()
        self.backtrack(result,nums,temp,target,start)
        return result

    def backtrack(self,result,nums,temp,target,start):
        if len(temp) == 4:
            if sum(temp) == target and temp not in result:
                result.append(temp[:])
            return 
        else:
            for i in range(start,len(nums)):
                temp.append(nums[i])
                self.backtrack(result,nums,temp,target,i+1)
                temp.pop()
#超时
class Solution:
    def fourSum(self, nums: List[int], target: int) -> List[List[int]]:
...
class Solution:
    def surfaceArea(self, grid: List[List[int]]) -> int:
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if i-1 >= 0:
                    overlap = min(grid[i][j],grid[i-1][j])*2
                    # print(overlap)
                    result -= overlap
                if j-1 >= 0:
                    overlap = min(grid[i][j],grid[i][j-1])*2
                    # print(overlap)
                    result -= overlap
        return result
```
...
#新节点的顺序是前序遍历顺序,把节点放列表里改指针也是原地操作
class Solution:
    def flatten(self, root: TreeNode) -> None:
        ...
        length = len(pre_list)
        for i in range(1,length):
            pre_list[i-1].left = None
            pre_list[i-1].right = pre_list[i]
            
    def pre_order(self,root,alist):
        if not root : return 
        alist.append(root)
        self.pre_order(root.left,alist)
        self.pre_order(root.right,alist)
```
...
class Solution:
    def addTwoNumbers(self, l1: ListNode, l2: ListNode) -> ListNode:
        ...
        while l1_stack or l2_stack or flag:
            l1_pop = l1_stack.pop() if l1_stack else 0
            l2_pop = l2_stack.pop() if l2_stack else 0
            temp = l1_pop + l2_pop + flag
            flag = temp // 10
...
class Solution:
    def gameOfLife(self, board: List[List[int]]) -> None:
        cache = [[0 for i in range(len(board[0]))] for i in range(len(board))]
        m = len(board)
        n = len(board[0])
        # print(cache)
        for i in range(m):
            for j in range(n):
                count = self.search_live_around(i,j,m,n,board)
                print(count)
                if board[i][j] == 1:
                    if count < 2: cache[i][j] == 0
                    elif count == 2 or count == 3 : cache[i][j] = 1
                    else : cache[i][j] = 0
                elif board[i][j] == 0:
                    if count == 3:
                        cache[i][j] = 1
    ...
    def search_live_around(self,x,y,m,n,board):
        temp = 0
...
class Solution:
    def solve(self, board: List[List[str]]) -> None:
        for i in range(1,len(board)-1):
            for j in range(1,len(board[0])-1):
                if board[i][j] == "O":
                    boolean,path = self.dfs(i,j,board)
                    if boolean:
                        self.convertOtoX(board,path)
    ...
    def dfs(self,i,j,board):
        ...
        while stack:
            ...
            for a,b in [(-1,0),(1,0),(0,-1),(0,+1)]:
                if 0<x+a<len(board)-1 and 0<y+b<len(board[0])-1 and board[x+a][y+b] == "O" and (x+a,y+b) not in seen:
    ...
    def convertOtoX(self,board,alist):
        for i,j in alist:
            board[i][j] = "X"
```
...
class Solution:
    ...
    def dfs(self,board,i,j):
        if 0<= i <= len(board)-1 and 0 <= j <= len(board[0])-1 and board[i][j] == "O":
            board[i][j] = "A"
...
class Solution:
    def addStrings(self, num1: str, num2: str) -> str:
        ...
        result = ""
        for i in range(len(num1)-1,-1,-1):
            temp = flag + int(num1[i]) + int(num2[i])
            if temp >= 10 :
                flag = 1
            else:
                flag = 0
